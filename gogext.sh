...
dd count=$offset if=$filename of=unpacker.sh bs=1 status=none
echo "Extracting MojoSetup archive as mojosetup.tag.gz"
dd skip=$offset count=$filesizes if=$filename of=mojosetup.tar.gz bs=1 status=none
#dd skip=$sum if=$filename of=data.zip ibs=1 obs=16384
echo "Extracting game files as data.zip (may take a while)"
dd skip=$(($offset+$filesizes)) if=$filename of=data.zip ibs=1 status=none
